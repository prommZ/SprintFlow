...
function getExpectedPeriods(createdAt, todayStr, frequency) {
...
function getCompletedPeriodsSet(habit, frequency) {
...
function calculateCurrentStreak(habit, todayStr) {
...
function calculateBestStreak(habit, todayStr) {
  const createdAt = habit.createdAt || new Date();
  const periods = getExpectedPeriods(createdAt, todayStr, habit.frequency);
  const completedSet = getCompletedPeriodsSet(habit, habit.frequency);
...
function calculateCompletionRate(habit, todayStr) {
  const createdAt = habit.createdAt || new Date();
  const periods = getExpectedPeriods(createdAt, todayStr, habit.frequency);
  const completedSet = getCompletedPeriodsSet(habit, habit.frequency);
...
function calculateOverallCompletion(habits, todayStr) {
